								...
								LPCTSTR a_lpszMessage)
{
	...
	else
	{
		...
		if (VirtualQuery(reinterpret_cast<LPCVOID>(Context->Eip), &MemInfo, sizeof(MemInfo)) &&
		...
		hprintf(l_hLogFile, _T("\r\nBytes at CS:EIP:\r\n"));
		LPBYTE code = reinterpret_cast<LPBYTE>(Context->Eip);
		...
		// Esp contains the bottom of the stack, or at least the bottom of
		// the currently used area
		LPDWORD pStack = reinterpret_cast<LPDWORD>(Context->Esp);
...
void GetPathAndModuleName(LPTSTR a_psPath, int a_iPathLen, LPTSTR a_psModule, int a_iModuleLen)
{
	...
	_tcsncpy(l_sFileName, l_psFilePart, _countof(l_sFileName) - 1);
	*l_psFilePart = _T('\0');
	_tcsncpy(a_psPath, l_sModule, a_iModuleLen - 1);
	...
	_tcsncpy(a_psModule, l_sFileName, a_iModuleLen - 1);
}