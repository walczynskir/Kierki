...
void DrawBitmap(HWND a_hWnd, HDC a_hDC, HBITMAP a_hBmp, int a_x, int a_y, const RECT& a_rectDest);
void DrawBitmapCropped(HWND a_hWnd, HDC a_hDC, HBITMAP a_hBmp, int a_x, int a_y, const RECT& a_rectDest);
void DrawBitmap(HWND a_hWnd, HDC a_hDC, HBITMAP a_hBmp, int a_xStart, int a_yStart);
...
LRESULT CALLBACK SelectBitmapProc(HWND a_hWnd, UINT a_iMsg, WPARAM a_wParam, LPARAM a_lParam) {
    SelectBitmapControl* ctrl = GetControl(a_hWnd);
...
void DrawAnimated(HWND a_hWnd, HDC a_hDC)
{
    ...
    if (l_pCtrl->m_enumAnimDirection == SelectBitmapControl::AnimDirection_Null)
    {
        if (HasWindowStyle(a_hWnd, SBM_STRETCH))
            DrawBitmap(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxCurrent].second, 0, 0, l_rect);
        else
            DrawBitmapCropped(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxCurrent].second, 0, 0, l_rect);
    }
    else
    {
        ...
        if (HasWindowStyle(a_hWnd, SBM_STRETCH))
        {
            DrawBitmap(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxTarget].second, 0, l_iOffset, l_rect);
            DrawBitmap(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxCurrent].second, l_iOffset, RectWidth(l_rect), l_rect);
        }
        else
        {
            DrawBitmapCropped(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxTarget].second, 0, l_iOffset, l_rect);
            DrawBitmapCropped(a_hWnd, a_hDC, l_pCtrl->m_mapBitmaps[l_pCtrl->m_idxCurrent].second, l_iOffset, RectWidth(l_rect), l_rect);
